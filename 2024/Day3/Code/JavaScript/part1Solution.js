...
while(mulIndex !== -1)
{
    ...
    let isSeparator = inputDay3[mulIndex + mulStart.length + firstDigit.length] === mulSeparator
    ...
    while(!isNaN(parseInt(inputDay3[mulIndex + mulStart.length + firstDigit.length + 1 + index])))
    {
        secondDigit += inputDay3[mulIndex + mulStart.length + firstDigit.length + 1 + index]
        index++
    }


    let isEnd = inputDay3[mulIndex + mulStart.length + firstDigit.length + secondDigit.length + 1] === mulEnd
    if(!isEnd){
        mulIndex = inputDay3.indexOf(mulStart, mulIndex + 1)
        continue
    }
