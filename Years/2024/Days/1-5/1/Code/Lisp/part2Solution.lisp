...
(defun main()
	(let ((data-chunks (read-file "input1.txt")))
		...
		(let ((column1 (get-column data-chunks 0)) (column2 (get-column data-chunks 1)))
			...
			(format t "~A" (get-total-similarity column1 column2))
		)
	)
)
...
(defun get-total-similarity (column1 column2)
	(let ((result (loop for i from 0 to (- (length column1) 1)
		collect (let ((number1 (nth i column1)))
			(* number1 (count-occurrences number1 column2))
		))))
...
(defun count-occurrences (number list)
  (if (null list)
      0
      (+ (if (= (car list) number) 1 0)
         (count-occurrences number (cdr list)))
	)
)
